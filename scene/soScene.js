...
import {PolygonLayer, GeoJsonLayer,ScatterplotLayer,ColumnLayer} from '@deck.gl/layers';
import SignalLayer from './signal-layer/signal-layer'
import SignalBuffer from './signal-layer/signal-buffer'
import {SIGNAL_DATA} from './signalData'
...
export class SoScene {
  ...
  addSignalLayer() {
    ...
    if(!this._singalLayer)
    {
      this._singalLayer= new SignalLayer({
        ...
        fadeTime: 8000,  // 信号消隐时长，默认为3000毫秒
        diffusionTimes:5,  // 扩散次数
        diffusionTime: 3000,  // 每次扩散时长，默认为1000毫秒
  ...
  pushSignal(signal) {
    ...
    //     OColor:DEFAULT_ORIGIN_COLOR,
    //     DColor:DEFAULT_DESTINATION_COLOR
    //   },{position:signal}
    // );

    // if(signalBuffer)
    // {
    //   signalBuffer.push(sigporps);
    //   TriggleTime=Date.now();
    // }
  }
}
